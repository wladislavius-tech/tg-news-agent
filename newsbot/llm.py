...
def compose_alert(text: str) -> str:
    ...
    headline, body = "❗️ Повітряна тривога", text[:400]
    if config.AI_AVAILABLE:
        data = _gemini_json(
            ...
            "Стисни у СУХИЙ короткий пост без емоцій і води: headline — один рядок, "
            "почни з ❗️, вкажи масштаб (конкретне місто чи вся країна) за фактами "
            "матеріалу; body — 1–3 дуже короткі речення, лише факти (що, де, скільки, "
            "джерело). «росія», «рф» — з малої. Не вигадуй нічого. "
