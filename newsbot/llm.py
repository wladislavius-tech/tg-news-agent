...
def is_same_event(title: str, alt_titles: list[str], recent_titles: list[str]) -> bool:
    ...
    data = _gemini_json(
        f"Нова новина: «{title}»\n{alt}\n\n"
        f"Нещодавно опубліковані пости каналу:\n{listing}\n\n"
        ...
        f"словами — звіряй місце, учасників, суть події. "
        f"ПРАВИЛО ОНОВЛЕНЬ: оновлення тієї ж події — НЕ дубль, ЛИШЕ якщо воно містить "
        f"цифри або факти, яких НЕМАЄ в опублікованих постах (зросла кількість жертв, "
        f"нове рішення, затримання). Якщо цифри й факти ТІ САМІ, що вже опубліковано — "
        f"це ДУБЛЬ, навіть якщо назва інша. "
