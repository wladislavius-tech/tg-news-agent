...
def run(dry_run: bool, force: bool) -> None:
    ...
    # РАННЯ перевірка розкладу — ДО дедупу, бо дедуп коштує AI-викликів.
    # Діагностика 19.08.2026: тригери приходять кожні 5 хв, а постимо раз на
    # 10-20 хв, тож більшість запусків марно палила квоту на classify_relation
    # для тих самих кандидатів — і до вечора всі 5 провайдерів віддавали 429.
    # Без AI не працює семантичний дедуп, і дублі йшли в канал (кейс Nord
    # Stream: два пости за 20 хв, спільне слово в заголовках лише одне).
    # Поріг тут беремо за НЕВІДФІЛЬТРОВАНИМ топом — це найкоротший можливий
    # інтервал, тобто найм'якша умова: пропустимо лише те, що точно ще рано.
    # Остаточну перевірку нижче (вже за очищеним списком) не чіпаємо.
    if candidates and not force:
        _elapsed = state_mod.minutes_since_regular_post(state, now)
        _seed = state.get("last_regular_post_at") or "genesis"
        if not allowed_to_post(now, _elapsed, candidates[0].related_count, _seed):
            log.info(
                "Ще рано постити (минуло %.0f хв) — пропускаю перевірку дублів, щоб не палити AI-квоту",
                _elapsed,
            )
            return
