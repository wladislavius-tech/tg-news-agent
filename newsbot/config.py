...
# Трохи менші за крок запусків cron-job.org, інакше межові секунди + випадковий
# зсув змушують пропускати цикли. Вдень фактичну кількість постів визначає САМЕ
# крок cron (інтервал тут уже менший за нього) — щоб постити вдень у 1.5 рази
# частіше, крок cron-job.org теж треба зменшити (30 хв → 20 хв, зовнішнє
# налаштування поза кодом). Вночі навпаки: інтервал БІЛЬШИЙ за крок cron, тож
# саме він і є обмежувачем — зменшення тут напряму дає ~1.5x нічних постів.
DAY_INTERVAL_HOT = 8       # вдень, якщо новина гаряча (менше за новий крок cron 20 хв)
DAY_INTERVAL_NORMAL = 12   # вдень, звичайна новина
NIGHT_INTERVAL_HOT = 73    # вночі, гаряча (було 110 → ~1.5x частіше)
NIGHT_INTERVAL_NORMAL = 110  # вночі, звичайна (було 165 → ~1.5x частіше)
